  ...
  async saveSubscription(subscription) {
    try {
      ...
      if (subError) {
        ...
        console.error('[SupabaseSubscriptionService] Error code:', subError.code);
        console.error('[SupabaseSubscriptionService] Error message:', subError.message);
        console.error('[SupabaseSubscriptionService] Error details:', subError.details);

        // Provide helpful error messages
        if (subError.code === '42P01') {
          throw new Error('Database table "subscriptions" does not exist. Please run the schema.sql file in your Supabase SQL Editor.');
        } else if (subError.code === '42501') {
          throw new Error('Database permission denied. Please check your SUPABASE_SERVICE_KEY has the correct permissions.');
        } else if (subError.message && subError.message.includes('Failed to fetch')) {
          throw new Error('Cannot connect to Supabase. Please check your SUPABASE_URL and internet connection.');
        }
