...
export const SubscriptionGuard: React.FC<SubscriptionGuardProps> = ({ children }) => {
  ...
  // Block access for expired trials - show payment wall instead of content
  // Only show for truly expired accounts, not for active paid accounts
  const shouldBlockAccess = (status === 'expired' && requiresPayment) ||
                            (billingOnly === true && status !== 'active') ||
                            (canUsePlatform === false && status !== 'active');

  // For non-billing pages with expired trial, show ONLY payment wall (block all access)
  if (shouldBlockAccess) {
    return (
      <PaymentWall 
        message={message || "Your trial has expired. Upgrade now to continue using all features."}
