      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.hasRefreshToken && data.refresh_token) {
            ...
            console.log('✅ Loaded permanent tokens from backend for user:', userIdToUse);
            console.log('✅ Token expires at:', new Date(data.expiry_date || Date.now() + 3600000).toISOString());

            // Start connection monitoring for 24/7 operation
            this.startConnectionMonitoring();
            return true;
          } else {
            console.log('⚠️ Backend has tokens but no refresh token available');
          }
        }
      } catch (backendError) {
